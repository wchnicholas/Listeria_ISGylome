...
def selecting_sites(MS_dict, rsa_dict, con_dict, poc_list, file_out):
  ...
  outfile.write("\t".join(['Site_ID','RSA_mean','RSA_sd','Lys_Conservation'])+"\n")
  for ID in MS_dict.keys():
    describe = MS_dict[ID]['describe']
    clusters = MS_dict[ID]['clusters']
    if 'cluster 1' not in clusters and 'cluster 2' not in clusters: continue
    for site in sorted(MS_dict[ID]['sites'],key=lambda x:int(x)):
      ...
      for PDB_site_identifier in rsa_dict[ID][site].keys():
        ...
        rsa_list.append(rsa)
      rsa_mean = np.mean(rsa_list)
      rsa_sd   = np.std(rsa_list)
      outfile.write("\t".join(map(str,[ID+'-'+site, rsa_mean, rsa_sd, con]))+"\n")
  outfile.close()

def main():
  file_out = 'result/ISG15_sites_cluster1and2.tsv'
